...
static HRESULT CreateD3DResources( _In_ ID3D12Device* d3dDevice,
                                   ...
                                   _Outptr_opt_ ID3D12Resource** texture,
									_Out_opt_ D3D12_SHADER_RESOURCE_VIEW_DESC* textureView )
{
    if ( !d3dDevice )
        return E_POINTER;
        ...
        case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
            {
                ...
                if (SUCCEEDED( hr ) && tex != nullptr)
                {
					...
					if(textureView)*textureView = SRVDesc;
...
static HRESULT CreateTextureFromDDS( _In_ ID3D12Device* d3dDevice,
                                     ...
                                     _In_ size_t maxsize,
                                     _In_ bool forceSRGB,
									_Outptr_opt_ ID3D12Resource** o_texture,
									_Outptr_opt_ ID3D12Resource** o_upload,
									_Out_opt_ D3D12_SHADER_RESOURCE_VIEW_DESC* textureView )
{
    HRESULT hr = S_OK;
	Microsoft::WRL::ComPtr<ID3D12Resource> texture, upload;
...

    {
		...
		if (SUCCEEDED(hr))
		{
			//InitializeTexture(GpuResource& Dest, UINT NumSubresources, D3D12_SUBRESOURCE_DATA SubData[])
			...
			d3dDevice->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
				...
				nullptr, MY_IID_PPV_ARGS(&upload));


			D3D12_RESOURCE_BARRIER BarrierDesc;
			BarrierDesc.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
			BarrierDesc.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
			BarrierDesc.Transition.pResource = texture.Get();
			BarrierDesc.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
			BarrierDesc.Transition.StateBefore = D3D12_RESOURCE_STATE_COMMON;
			BarrierDesc.Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_DEST;
			cmdList->ResourceBarrier(1, &BarrierDesc);
			
			UpdateSubresources(cmdList, texture.Get(), upload.Get(), 0, 0, subresourceCount, initData.get());
	...
	if (SUCCEEDED(hr)){
		*o_texture = texture.Detach();
		*o_upload = upload.Detach();
	}
    return hr;
}
	...
	DDS_ALPHA_MODE* alphaMode )
{
    ...
    HRESULT hr = CreateTextureFromDDS( d3dDevice, cmdList,
                                       header, ddsData + offset, ddsDataSize - offset, maxsize,
                                       forceSRGB, texture, upload, textureView );
    if ( SUCCEEDED(hr) )
    {
	...
	DDS_ALPHA_MODE* alphaMode )
{
    ...
    hr = CreateTextureFromDDS( d3dDevice, cmdList,
